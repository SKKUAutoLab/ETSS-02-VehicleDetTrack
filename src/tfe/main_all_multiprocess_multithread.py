...
def run_node(_, index_node):
	...
	for index, config in enumerate(config_files):
		...
		if index % args.nodes == index_node:
			...
			logger.success(f"Total camera init time: {total_camera_init_time} seconds.")
			logger.success(f"Actual processing time: {total_process_time - total_camera_init_time} seconds.")
...
def main():
	...
	logger.success(f"Main multiprocess Multithread time: {main_total_time} seconds.")
